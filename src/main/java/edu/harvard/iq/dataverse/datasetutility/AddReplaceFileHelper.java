    ...
    private boolean step_070_run_update_dataset_command() {
        ...
        if (isFileReplaceOperation()) {
            ...
            if (!multifile) {
                ...
                update_cmd = new UpdateDatasetVersionCommand(dataset, dvRequest, newlyAddedFileMetadatas, filesToDelete, clone);
            }
        } else {
            update_cmd = new UpdateDatasetVersionCommand(dataset, dvRequest, newlyAddedFileMetadatas, clone);
        }
        if (!multifile) {
    ...
    public Response addFiles(String jsonData, Dataset dataset, User authUser) {
        ...
        try {
            ...
            if (filesJson != null) {
                ...
                try {
                    Command<Dataset> cmd = new UpdateDatasetVersionCommand(dataset, dvRequest, newlyAddedFileMetadatas, clone);
