...
public class XmlMetadataTemplate {
    ...
    public void writeCreators(XMLStreamWriter xmlw, List<DatasetAuthor> authorList, boolean deaccessioned) throws XMLStreamException {
        ...
        if (authorList != null && !authorList.isEmpty()) {
            for (DatasetAuthor author : authorList) {
                ...
                List<Pair<String,String>> affList = new ArrayList<>();
                if (author.getAffiliation() != null && !author.getAffiliation().getValue().isEmpty()) {
                    if(author.getAffiliationId() != null && !author.getAffiliationId().getValue().isEmpty())
                        affList.add(new ImmutablePair<>(author.getAffiliation().getValue(), author.getAffiliationId().getValue()));
                    else
                        affList.add(new ImmutablePair<>(author.getAffiliation().getValue(), null));
                }
                if (author.getAffiliation2() != null && !author.getAffiliation2().getValue().isEmpty()) {
                    if(author.getAffiliation2Id() != null && !author.getAffiliation2Id().getValue().isEmpty())
                        affList.add(new ImmutablePair<>(author.getAffiliation2().getValue(), author.getAffiliation2Id().getValue()));
                    else
                        affList.add(new ImmutablePair<>(author.getAffiliation2().getValue(), null));
                }
                if (author.getAffiliation3() != null && !author.getAffiliation3().getValue().isEmpty()) {
                    if(author.getAffiliation3Id() != null && !author.getAffiliation3Id().getValue().isEmpty())
                        affList.add(new ImmutablePair<>(author.getAffiliation3().getValue(), author.getAffiliation3Id().getValue()));
                    else
                        affList.add(new ImmutablePair<>(author.getAffiliation3().getValue(), null));
                }
                ...
                if (StringUtils.isNotBlank(creatorName)) {
                    ...
                    writeEntityElements(xmlw, "creator", null, creatorObj, affList, nameIdentifier, nameIdentifierScheme);
                }
    ...
    private void writeContributors(XMLStreamWriter xmlw, DvObject dvObject) throws XMLStreamException {
        ...
        for (DatasetFieldCompoundValue producerFieldValue : compoundProducers) {
            ...
            for (DatasetField subField : producerFieldValue.getChildDatasetFields()) {
                ...
                case DatasetFieldConstant.producerAffiliation:
                    affList.add(new ImmutablePair<>(subField.getValue(),null));
                    break;
                }
            }
            if (StringUtils.isNotBlank(producer)) {
                ...
                writeEntityElements(xmlw, "contributor", "Producer", entityObject, affList, null, null);
            }
        ...
        for (DatasetFieldCompoundValue distributorFieldValue : compoundDistributors) {
            ...
            if (StringUtils.isNotBlank(distributor)) {
                ...
                writeEntityElements(xmlw, "contributor", "Distributor", entityObject, affList, null, null);
            }
        ...
        for (DatasetFieldCompoundValue contactFieldValue : compoundContacts) {
            ...
            if (StringUtils.isNotBlank(contact)) {
                ...
                writeEntityElements(xmlw, "contributor", "ContactPerson", entityObject, affList, null, null);
            }
    ...
    private void writeEntityElements(XMLStreamWriter xmlw, String elementName, String type, JsonObject entityObject, List<Pair<String,String>> affiliations, String nameIdentifier, String nameIdentifierScheme) throws XMLStreamException {
        ...
        if (affiliations != null) {
            for (Pair<String, String> affiliation : affiliations)
                if (StringUtils.isNotBlank(affiliation.getKey())) {
                    attributeMap.clear();
            /*
            boolean isROR=false;
          ...
          */
                    if (StringUtils.isNotBlank(affiliation.getValue())) {

                        attributeMap.put("schemeURI", "https://ror.org");
                        attributeMap.put("affiliationIdentifierScheme", "ROR");
                        attributeMap.put("affiliationIdentifier", affiliation.getValue());
                    }

                    XmlWriterUtil.writeFullElementWithAttributes(xmlw, "affiliation", attributeMap, StringEscapeUtils.escapeXml10(affiliation.getKey()));
                }
        }
        xmlw.writeEndElement();
    }
